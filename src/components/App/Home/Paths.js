        ...
        {(loading || loadingTheft) ? <LoadingText>Fetching Paths</LoadingText> :
          <UL>
            {Object.keys(allPaths).map((key) => {
              ...
              return <li>
                <PathItem summary={summary} parent={true} to={isIssuePath ? `/path/${get(filterParams, 'initPath')}/%2F${key}` : `/path/${get(filterParams, 'initPath')}/${key}`} name={allPaths[key]['display_name'] ? allPaths[key]['display_name'] : startCase(key || 'N/A')} />
                {!isEmpty(childPaths) ? <ul>
                  {Object.keys(childPathsClone).map((innerKey) => {
                    const isIssue = childPaths[innerKey] && childPaths[innerKey].leaf
                    return <li><PathItem summary={summary} to={isIssue ? `/path/${get(filterParams, 'initPath')}%2F${key}%2F${innerKey}` : `/path/${get(filterParams, 'initPath')}%2F${key}%2F${innerKey}`} name={(childPaths[innerKey]['metadata'] && childPaths[innerKey]['metadata']['display_name']) || childPaths[innerKey]['display_name'] || startCase(innerKey || 'N/A')} /></li>
                  })}
                </ul> : null}
              </li>
            })}
