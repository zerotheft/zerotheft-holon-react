...
const PathList = ({ history, match }) => {
  ...
  return <Wrapper>
    ...
    <Container style={{ maxWidth: 1100 }}>
      <ListWrapper>
        {get(nations, 'data', []).map(i => <div>
          <div className='issue'>
            <div style={{ width: 410 }}>
              <FontAwesomeIcon icon={faFolder} style={{ marginRight: 5, color: colors.primary }} />
              <span style={{ cursor: 'pointer' }} onClick={() => history.push(`/path/${i.nation}`)}>{i.nation || 'N/A'}</span>
            </div>
            ...
            <Button onClick={() => history.push(`/pathReport/${i.nation}`)} plain><FontAwesomeIcon icon={faFile} style={{ fontSize: 16, marginRight: 5 }} />View Report</Button>
          </div>
        </div>)}
        {!get(nations, 'data', []).length && <div style={{ padding: 10, fontStyle: 'italic' }}>
          Empty Path
        </div>}
      </ListWrapper>
    </Container>
